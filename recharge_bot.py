...
if not (TG_BOT_TOKEN and SUPABASE_URL and SUPABASE_KEY and MP_ACCESS_TOKEN and PUBLIC_BASE_URL):
    raise SystemExit(
        ...
        "TG_RECHARGE_BOT_TOKEN, SUPABASE_URL, SUPABASE_ANON_KEY (o SUPABASE_API_KEY), "
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Recibirás un link de pago (y un QR opcional). "
        "Cuando el pago sea aprobado, tus créditos se acreditan automáticamente."
    )
...
def mp_webhook():
    ...
    try:
        ...
        elif body.get("data", {}).get("id"):  # compatibilidad
            payment_id = str(body["data"]["id"])
        ...
        if status == "approved":
            ...
            # Notificar al usuario usando el scheduler del bot (thread-safe)
            tg_app.create_task(
...
if __name__ == "__main__":
    ...
    # Levantamos el servidor HTTP (Flask) en segundo plano,
    # y el bot de Telegram en primer plano para mantener vivo el proceso.
    def run_http():
